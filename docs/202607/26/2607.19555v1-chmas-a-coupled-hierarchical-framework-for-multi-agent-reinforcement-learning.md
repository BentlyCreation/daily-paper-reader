...
tags: ["query:lbm"]
score: 8.0
evidence: 分层MARL框架，适用于路径查找
tldr: 多智能体强化学习面临全局协调与局部执行的平衡挑战。本文提出耦合层次化多智能体系统框架，将决策分解为中央战略规划与分布式战术执行，并通过反馈机制实现双向信息流动。采用异步更新协议解决层次化学习的非平稳性，理论证明战略层收敛率为O(logK/√K)。在觅食环境中，智能体成功学习空间分区探索策略，验证了框架的稳定性和有效性。
source: arxiv
...
motivation: 解决多智能体系统中全局协调与局部执行的平衡难题，以及层次化学习中的非平稳性问题。
method: 构建耦合层次化框架，战略层整合全局状态生成每T步指导，战术层结合指导和局部观察执行；通过耦合系数λ实现反馈，并采用异步更新协议稳定训练。
result: 理论证明战略层以O(logK/√K)速率收敛；在觅食任务中，智能体学会空间分区探索策略，双层稳定收敛。
conclusion: CHMAS框架通过双向信息流和异步更新有效协调多智能体，实现了稳定的层次化学习。
---

## 摘要
多智能体强化学习系统在平衡跨不同时间尺度的全局协调与局部执行方面面临根本性挑战。本文介绍了耦合层次多智能体系统（CHMAS），这是一种新颖的框架，它将多智能体决策分解为中心化战略规划与分布式战术执行，并具有双向信息流。战略层整合所有智能体的状态以及专属的全局环境状态，每T个时间步生成引导动作，而战术智能体则执行由战略引导和局部邻域观测增强的分布式策略。与现有单向控制的层次方法不同，CHMAS建立了一种反馈机制，其中累积的战术奖励通过耦合系数λ影响战略目标，确保战略计划始终基于战术可行性。为解决层次学习中固有的非平稳性问题，我们提出了一种异步更新协议，其中战略参数每N_f个战术回合更新一次，使得战术策略能够在战略变化之间收敛到准平稳点。我们提出了一个捕获完整系统动力学的一般双层公式，以及一个可处理的加性近似，以便进行严格分析。理论分析证明，在标准假设下，该异步方案在K次战略更新后，战略层实现了O(log K/√K)的收敛速率。在多智能体觅食领域的实验验证表明，尽管存在层次耦合，该方案仍能成功学习空间分割的探索策略，且两层均稳定收敛。
